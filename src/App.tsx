...
const lotteryLinkTarget = /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.userAgent.includes('Mac') && navigator.maxTouchPoints > 1) ? undefined : '_blank'
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    {queueOpen && currentQueue && <QueueDialog onClose={() => setQueueOpen(false)}><div className="queue-modal"><button className="modal-close" onClick={() => setQueueOpen(false)}>關閉</button><PriorityBadge priority={currentQueue.product.priority} /><p>下一筆 · 尚有 {highQueue.length} 筆</p><h2 id="queue-title">{currentQueue.product.name}</h2><div className="queue-store"><StoreIcon /><div><strong>{storeMap.get(currentQueue.entry.storeId)?.name}</strong><span>{storeMap.get(currentQueue.entry.storeId)?.priority}</span></div></div><PurchaseInfo entry={currentQueue.entry} /><a className="modal-primary" href={currentQueue.entry.lotteryUrl} target={lotteryLinkTarget} rel="noreferrer" onClick={() => setStatus(currentQueue.entry.id, 'entered')}><ExternalLink />前往 LINE 抽選</a><p className="dialog-note">開啟連結後會自動標記已抽，請在 LINE 完成登記。</p><div className="modal-actions"><button onClick={() => setQueueOpen(false)}>稍後處理</button><button onClick={() => setStatus(currentQueue.entry.id, 'entered')}><Check />標記已抽，下一筆</button></div></div></QueueDialog>}
  </div>
}
...
function StoreView({ states, setStatus }: { states: DrawStateMap; setStatus: (id: string, status: 'entered' | 'not-entered') => void }) {
  ...
  return <section className="page"><h2>依店家查看</h2><p>先看店家的本輪商品，再逐筆前往抽選。</p><div className="store-list">{published.map(store => { const entries = lotteries.filter(entry => entry.storeId === store.id); const done = entries.filter(entry => isEntered(states, entry.id)).length; return <article key={store.id}><button className="store-summary" aria-expanded={open === store.id} onClick={() => setOpen(open === store.id ? null : store.id)}><span><strong>{store.name}</strong><small>{store.priority} · {store.city}</small></span><span>{done} / {entries.length} 已抽</span>{open === store.id ? <ChevronUp /> : <ChevronDown />}</button>{open === store.id && <div className="store-products"><PurchaseInfo entry={entries.find(entry => entry.purchaseNote)} />{entries.sort((a, b) => { const productA = products.find(p => p.id === a.productId); const productB = products.find(p => p.id === b.productId); return PRIORITY_WEIGHT[productB?.priority || '未分類'] - PRIORITY_WEIGHT[productA?.priority || '未分類'] || (productA?.sortOrder ?? 999999) - (productB?.sortOrder ?? 999999) }).map(entry => { const product = products.find(item => item.id === entry.productId)!; const done = isEntered(states, entry.id); return <div className="store-product" key={entry.id}><PriorityBadge priority={product.priority} /><span><strong>{product.name}</strong><small>{done ? '已抽 ✓' : '尚未抽'}</small></span>{done ? <button onClick={() => setStatus(entry.id, 'not-entered')}>取消</button> : <a href={entry.lotteryUrl} target={lotteryLinkTarget} rel="noreferrer" onClick={() => setStatus(entry.id, 'entered')}>抽選</a>}</div>})}</div>}</article>})}</div></section>
}
