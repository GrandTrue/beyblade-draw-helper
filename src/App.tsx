...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    {queueOpen && currentQueue && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="高優先抽選佇列"><div className="queue-modal"><button className="modal-close" onClick={() => setQueueOpen(false)}>關閉</button><PriorityBadge priority={currentQueue.product.priority} /><p>下一筆 · 尚有 {highQueue.length} 筆</p><h2>{currentQueue.product.name}</h2><div className="queue-store"><StoreIcon /><div><strong>{storeMap.get(currentQueue.entry.storeId)?.name}</strong><span>{storeMap.get(currentQueue.entry.storeId)?.priority}</span></div></div><a className="modal-primary" href={currentQueue.entry.lotteryUrl} target="_blank" rel="noreferrer"><ExternalLink />前往 LINE 抽選</a><div className="modal-actions"><button onClick={() => setQueueOpen(false)}>稍後</button><button onClick={() => setStatus(currentQueue.entry.id, 'entered')}><Check />已抽，下一筆</button></div></div></div>}
  </div>
}
