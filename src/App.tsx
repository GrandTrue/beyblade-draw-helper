...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    {queueOpen && currentQueue && <QueueDialog onClose={() => setQueueOpen(false)}><div className="queue-modal"><button className="modal-close" onClick={() => setQueueOpen(false)}>關閉</button><PriorityBadge priority={currentQueue.product.priority} /><p>下一筆 · 尚有 {highQueue.length} 筆</p><h2 id="queue-title">{currentQueue.product.name}</h2><div className="queue-store"><StoreIcon /><div><strong>{storeMap.get(currentQueue.entry.storeId)?.name}</strong><span>{storeMap.get(currentQueue.entry.storeId)?.priority}</span></div></div><PurchaseInfo entry={currentQueue.entry} /><a className="modal-primary" href={currentQueue.entry.lotteryUrl} target="_blank" rel="noreferrer" onClick={() => setStatus(currentQueue.entry.id, 'entered')}><ExternalLink />前往 LINE 抽選</a><p className="dialog-note">開啟連結後會自動標記已抽，請在 LINE 完成登記。</p><div className="modal-actions"><button onClick={() => setQueueOpen(false)}>稍後處理</button><button onClick={() => setStatus(currentQueue.entry.id, 'entered')}><Check />標記已抽，下一筆</button></div></div></QueueDialog>}
  </div>
}
